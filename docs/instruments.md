...
| `bodyaudit`, `footaudit`, `walkaudit`, `neckaudit`, `column`, `jawprobe`, `envelope` | body regions: proportions, sole contact and the foot in its own frame (width, wedge, medial bend, toe-out — `tests/feet.rs` asserts them), gait excursions, neck spans and the junction (collar rim, nape ledge, shoulder slope — `tests/throat.rs` asserts them), midline profiles, jaw shelf, exploration-range envelopes |
| `render -- --far 12`, `--tier far` | the distance tier (#350): `--far` draws each view at the pixels a 1080-line, 60-degree camera spends on it that many metres off, scaled back up so sheets stitch (proved byte-identical at the near framing); `--tier far` builds the far tier and draws it in the hair's place, `--tier near` builds it and draws the near hair - the control that asking for a far tier moves no byte; `--budget` prints both tiers |
